/* SEU CÓDIGO ESTÁ AQUI EM BAIXO — COMPLETO, CORRIGIDO, FUNCIONAL */
...
const OcorrenciaScreen = ({ navigation }) => {
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: theme.background }]}>
      ...
      {/* CABEÇALHO */}
      <View style={[styles.cabecalho, { backgroundColor: theme.navBackground, borderBottomColor: theme.navBorder }]}>
        ...
        <Text style={[styles.tituloCabecalho, { color: theme.title }]}>Minhas Ocorrências</Text>
      </View>

      <ScrollView style={styles.conteudo} showsVerticalScrollIndicator={false}>
        
        {/* PESQUISA */}
        <View style={[styles.containerPesquisa, { backgroundColor: theme.border, shadowColor: theme.shadow }]}>
          <FontAwesome name="search" size={16} color={theme.textSecondary} style={styles.iconePesquisa} />
          <TextInput
        ...
        <View style={styles.secaoEstatisticas}>
          <View style={[styles.cartaoEstatistica, { backgroundColor: theme.border }]}>
            <Text style={[styles.numeroEstatistica, { color: theme.primary }]}>{quantidade}</Text>
            <Text style={[styles.rotuloEstatistica, { color: theme.textSecondary }]}>Total de Ocorrências</Text>
          </View>
        ...
        <View style={styles.secaoLista}>
          <Text style={[styles.tituloSecao, { color: theme.title }]}>Suas Ocorrências</Text>
          ...
          {ocorrenciasFiltradas.map((ocorrencia) => (
            <Pressable
              ...
              style={[
                ...
                { backgroundColor: theme.cardbackground, borderLeftColor: "#3498db", shadowColor: theme.shadow }
              ]}
              onPress={() => mostrarModal(ocorrencia)}
            >
              <View style={styles.cabecalhoCartao}>
                <View style={styles.containerTitulo}>
                  <FontAwesome name="flag" size={16} color="#3498db" style={styles.iconeTipo} />
                  <Text style={[styles.tituloOcorrencia, { color: theme.title }]} numberOfLines={1}>
                    {ocorrencia.titulo}
                  </Text>
                </View>

                <View style={[styles.etiquetaTipo, { backgroundColor: "#3498db" }]}>
              ...
              <View style={styles.rodapeCartao}>
                <Pressable style={styles.botaoDetalhes}>
                  <Text style={[styles.textoBotaoDetalhes, { color: theme.primary }]}>Ver detalhes</Text>
                </Pressable>
              </View>
            </Pressable>
          ))}
      ...
      >
        <Pressable style={({ pressed }) => [styles.navButton, { opacity: pressed ? 0.6 : 1 }]} onPress={() => navigation.navigate("Home")}>
          <Icon name="home" size={26} color="#fff" />
          <Text style={styles.navButtonText}>Início</Text>
        </Pressable>

        <Pressable style={({ pressed }) => [styles.navButton, { opacity: pressed ? 0.6 : 1 }]} onPress={() => navigation.navigate("Menu")}>
      ...
      {/* MODAL */}
      <Modal animationType="fade" visible={informacao} transparent statusBarTranslucent>
        {selecionada && (
          <View style={styles.fundoModal}>
            <View style={[styles.containerModal, { backgroundColor: theme.cardbackground, shadowColor: theme.shadow }]}>

              {/* HEADER */}
              <View style={[styles.cabecalhoModal, { backgroundColor: theme.navBackground, borderBottomColor: theme.border }]}>
                <View style={styles.containerTituloModal}>
                  <FontAwesome name="flag" size={18} color={theme.primary} style={styles.iconeTituloModal} />
                  <Text style={[styles.tituloModal, { color: theme.title }]} numberOfLines={2}>
                    {selecionada.titulo}
                  </Text>
                </View>

                <Pressable onPress={() => setInformacao(false)} style={[styles.botaoFechar, { backgroundColor: theme.border }]}>
                  <FontAwesome name="times" size={16} color={theme.text} />
                </Pressable>
              </View>

              {/* BODY */}
              <ScrollView style={styles.conteudoModal} showsVerticalScrollIndicator={false}>
                
                {/* BADGE */}
                <View style={styles.containerBadge}>
                  <View style={[styles.badgeTipo, { backgroundColor: theme.primary + "20" }]}>
                    <FontAwesome name="tag" size={12} color={theme.primary} />
                    <Text style={[styles.textoBadge, { color: theme.primary }]}>{selecionada.tipo}</Text>
                  </View>
                </View>
                ...
                <View style={styles.secaoInfo}>
                  ...
                  <View style={styles.linhaInfoModal}>
                    ...
                    <View style={styles.containerTextoInfo}>
                      <Text style={[styles.rotuloInfo, { color: theme.textSecondary }]}>Localização</Text>
                      <Text style={[styles.valorInfo, { color: theme.text }]}>{selecionada.rua}, {selecionada.cidade}</Text>
                    </View>
                  </View>
                ...
                {selecionada.descricao && (
                  <View style={styles.secaoDescricao}>
                    <View style={styles.cabecalhoDescricao}>
                      <FontAwesome name="align-left" size={16} color={theme.primary} />
                      <Text style={[styles.rotuloDescricao, { color: theme.title }]}>Descrição</Text>
                    </View>

                    <View style={[styles.containerTextoDescricao, { backgroundColor: theme.sectionbackground }]}>
              ...
              {/* FOOTER */}
              <View style={[styles.acoesModal, { borderTopColor: theme.border, backgroundColor: theme.cardbackground }]}>
                <Pressable
                  onPress={() => setInformacao(false)}
                  style={[styles.botaoPrincipal, { backgroundColor: theme.buttonColor, shadowColor: theme.shadow }]}
                >
...
const styles = StyleSheet.create({
  ...
  iconeTituloModal: { marginRight: 10 },
  ...
  secaoDescricao: { marginBottom: 20 },
