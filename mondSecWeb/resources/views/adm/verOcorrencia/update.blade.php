...
<body>
    <div class="container">
        <h2>Alterar Dados da Ocorrencia</h2>
        ...
        <form id="formUpdate" action="{{ route('adm.ocorrencia.update', $ocorrencia->id) }}" method="POST">
            ...
            <input type="text" id="nome" name="nome" value="{{ old('nome', $ocorrencia>nome) }}">
            @error('nome')
                <div class="error">{{ $message }}</div>
            @enderror

            <label for="email">Email:</label>
            <input type="email" id="email" name="email" value="{{ old('email', $ocorrencia->email) }}">
            @error('email')
                <div class="error">{{ $message }}</div>
            @enderror

            <label for="telefone">Telefone:</label>
            <input type="telefone" id="telefone" name="telefone" value="{{ old('telefone', $ocorrencia->telefone) }}">
            @error('telefone')
                <div class="error">{{ $message }}</div>
            @enderror
